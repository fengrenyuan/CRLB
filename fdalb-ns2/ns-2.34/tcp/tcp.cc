	...
	flow_id_(0)
{
	...
	//ws: added for D2TCP
	bind("d2tcp_", &d2tcp_);
	bind("d2tcp_gamma_", &d2tcp_gamma_);
	bind("d2tcp_deadline_", &d2tcp_deadline_);
	bind("d2tcp_total_bytes_", &d2tcp_total_bytes_);
  //ws: added for SAB
  bind("enable_SAB_", &enable_SAB_);

	//liu: added for CRLB
	bind("flow_id_", flow_id_);
...
TcpAgent::delay_bind_init_all()
{
	...
	//ws: added for D2TCP
	delay_bind_init_one("d2tcp_");
	delay_bind_init_one("d2tcp_gamma_");
	delay_bind_init_one("d2tcp_deadline_");
  //ws: added for SAB
  delay_bind_init_one("enable_SAB_");

	//liu: added for CRLB
	delay_bind_init_one("flow_id_");
...
TcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
	...
	if (delay_bind(varName, localName, "ecnhat_tcp_friendly_", &ecnhat_tcp_friendly_, tracer)) return TCL_OK;

	//ws: added for D2TCP
	if (delay_bind(varName, localName, "d2tcp_", &d2tcp_ , tracer)) return TCL_OK;
	if (delay_bind(varName, localName, "d2tcp_gamma_", &d2tcp_gamma_ , tracer)) return TCL_OK;
	if (delay_bind(varName, localName, "d2tcp_deadline_", &d2tcp_deadline_ , tracer)) return TCL_OK;
	if (delay_bind(varName, localName, "d2tcp_total_bytes_", &d2tcp_total_bytes_ , tracer)) return TCL_OK;
  //ws: added for SAB
  if (delay_bind_bool(varName,localName,"enable_SAB_",&enable_SAB_,tracer))return TCL_OK;

	//liu: added for CRLB
	if (delay_bind_bool(varName,localName,"flow_id_",&flow_id_,tracer)) return TCL_OK;
...
void TcpAgent::output(int seqno, int reason)
{
  ...
  double win = windowd();
	...
	Packet* p = allocpkt();
  int size_local = hdr_cmn::access(p)->size();
  ...
  //add by zhj
  if(win < 1){
    size_local = size_local * win;
    hdr_cmn::access(p)->size() = size_local;
    //printf("output() size() %d\n", hdr_cmn::access(p)->size());
  }
	int databytes = hdr_cmn::access(p)->size();
	tcph->seqno() = seqno;

  //added by zhj for LTPB
  tcph->rwnd_ = 0.0;
...
//		this->d2tcp_deadline_ = atof(argv[2]);
      this->DSAB_deadline_ = atof(argv[2]);
      return (TCL_OK);
    }
...
int TcpAgent::window()
{
	 ...
	 * for detecting spurious retransmission timeouts.
		...
		//zhj return (cwnd_ < wnd_ ? (int)cwnd_ : (int)wnd_);
//          return (int)wnd_;
          //ws added for SAB
...
{
	// zhj return (cwnd_ < wnd_ ? (double)cwnd_ : (double)wnd_);
//  return (double)wnd_;
  //ws: added for SAB
  if(enable_SAB_||DSAB_deadline_>0)
    return (double)wnd_;
...
//	int win = window();
  double win=windowd();
	...
	if (burstsnd_timer_.status() == TIMER_PENDING)
		return;
  //modified by zhj for ltpb
	while ((t_seqno_ <= highest_ack_ + win && t_seqno_ < curseq_)|| (win < 1 && t_seqno_ <= highest_ack_+ win + 1)) {
...
TcpAgent::slowdown(int how)
{
	...
	else if (how & CLOSE_SSTHRESH_D2TCP)
	{
		if(d2tcp_gamma_ > 0){
			ssthresh_ = (int)(cwnd_*(1-d2tcp_gamma_/2.0));
		}
		else {
	...
	if (how & CLOSE_CWND_HALF) {
		...
		if(d2tcp_gamma_ > 0) {
			cwnd_ = cwnd_ * (1 - d2tcp_gamma_ / 2.0);
		}
		else
			cwnd_ = cwnd_+1;
	}
	...
	if (how & (CLOSE_CWND_HALF|CLOSE_CWND_RESTART|CLOSE_CWND_INIT|CLOSE_CWND_ONE|CLOSE_CWND_ECNHAT|CLOSE_CWND_D2TCP))
		cong_action_ = TRUE;
...
void TcpAgent::ecn(int seqno)
{
	if (seqno > recover_ ||  
		...
		//ws: added for D2TCP
		if(d2tcp_ && d2tcp_deadline_>0)
			slowdown(CLOSE_CWND_D2TCP|CLOSE_SSTHRESH_D2TCP);
		else if (ecnhat_) {
