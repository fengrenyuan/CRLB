...
//				iph->prio() = iph->prio() / 40 * 1000 + set_prio(seqno, curseq_) / 1460;
  ...
  if(enable_SAB_ && DSAB_deadline_ <= 0)
  {
    if (this->type_ != PT_ACK)
    {
			if(SAB_state_==1) {
				hdr_cmn::access(p)->ptype() = PT_LTPB;
				tcph->DSAB_SYN_FIN_ = this->DSAB_SYN_FIN_;
				this->DSAB_SYN_FIN_ = 0;
			}
			if (seqno+datalen>=curseq_ && DSAB_SYN_FIN_==0 && signal_on_empty_)
			//if (DSAB_FIN==1 && DSAB_SYN_FIN_==0)
			{
				if(tcph->DSAB_SYN_FIN_==1)
				{
					tcph->DSAB_SYN_FIN_ = 0;
				}
				else
					tcph->DSAB_SYN_FIN_ = -1;
				DSAB_SYN_FIN_ = -1;
				SAB_state_ = -1;
				DSAB_FIN = 0;
			}
      tcph->DSAB_type_ = DSAB_SAB_PACKET;
      tcph->rwnd_=10000;
    }else
    {
      tcph->rwnd_=rwnd_;
  ...
  if(DSAB_deadline_>0)
  {
    if(this->type_!=PT_ACK)
    {
      double timeLeft= start_time+DSAB_deadline_-now();
      int buffered_bytes = (infinite_send_) ? TCP_MAXSEQ :
                           curseq_ - highest_ack_ + 1;
      double rate=1000000000;
      if(timeLeft>0){
        rate=buffered_bytes*8/timeLeft;
	  	}

      /*SAB_state_:
       * 1   send R-r
       * 2   send BC confirmation packets
       * 3   send normol packets or send R-w packets.
       * 4   pending: a R-r or R-w just send and not recv feedback.
       * -1   send BR packets
       * */
			bool last_packet = seqno + datalen >= curseq_;

			tcph->DSAB_type_ = DSAB_NORMAL;
      if (SAB_state_ == DSAB_STATE_SEND_RR) // syn send, send R-r packet
      {
				if(last_packet)
				{
					this->SAB_state_ = DSAB_STATE_CLOSE;
				} else {
					hdr_cmn::access(p)->ptype() = PT_LTPB;
					tcph->DSAB_wndApply_ = rate;
					tcph->DSAB_wnd_ = rate;
					tcph->DSAB_type_ = DSAB_R_R;
					this->rateOld_ = rate;
					this->SAB_state_ = DSAB_STATE_WAIT_RW;
				}
      }else if (SAB_state_ == DSAB_STATE_SEND_BC) //recv feedback, send confirmation
      {
				if(last_packet)
				{
					hdr_cmn::access(p)->ptype() = PT_LTPB;
					tcph->DSAB_wndApply_ = this->wndApply_;
					tcph->DSAB_wnd_ = this->wnd_ - wndBase_;
					tcph->DSAB_total_wnd_ = this->wnd_;
					tcph->DSAB_type_ = DSAB_BCBR;
					this->SAB_state_ = DSAB_STATE_CLOSE;     //send a BC and can send R-w.
				} else {
					hdr_cmn::access(p)->ptype() = PT_LTPB;
					tcph->DSAB_wndApply_ = this->wndApply_;
					tcph->DSAB_wnd_ = this->wnd_ - wndBase_;
					tcph->DSAB_type_ = DSAB_BC;
					this->SAB_state_ = DSAB_STATE_SEND_RW;     //send a BC and can send R-w.
				}
      }else if (last_packet&&SAB_state_==DSAB_STATE_SEND_NOTHING||SAB_state_==DSAB_STATE_SEND_RW) // fin, send BR packet, and can't send R-r or R-w
		  //change wndApply_ to wnd_
      {
        hdr_cmn::access(p)->ptype() = PT_LTPB;
        tcph->DSAB_wndApply_ = this->wnd_;
        tcph->DSAB_wnd_ = this->wnd_;
        tcph->DSAB_type_=DSAB_BR;
				this->SAB_state_ = DSAB_STATE_CLOSE;
      } else if (SAB_state_ == DSAB_STATE_SEND_RW)
      {
				if(last_packet)
				{
					this->SAB_state_ = DSAB_STATE_CLOSE;
				} else if (rate > this->rateOld_ * 1.05) //send R-w if rate changed.
        {
          this->wndApply_ = wnd_ * rate / rateOld_;
          wndBase_ = wnd_;
          this->wndApply_ -= wndBase_; //send window varation.
          hdr_cmn::access(p)->ptype() = PT_LTPB;
          tcph->DSAB_wndApply_ = this->wndApply_;
          tcph->DSAB_wnd_ = this->wndApply_;
    ...
    } else
    {
            tcph->DSAB_wnd_ = this->wnd_;
            tcph->DSAB_wndApply_ = this->wndApply_;
            tcph->DSAB_type_ = this->DSAB_type_;
    }
  }
	send(p, 0);
...
//    }	
	if ((signal_on_empty_) && (!buffered_bytes) && (!syn)) {
		...
		rcv_nxt_;
//		sendpacket(seqno, rcv_nxt_, TH_ACK, 0, reason);
		bufferempty();
	}
	//
	...
	//
	// this is an optionls that causes us to slow-start if we've
...
//     }
	...
	if (datalen > 0) {
		...
		if (datalen == 1 && prob_mode_)
			goto send;
    //ws: added for SAB, if have data send
    if ((enable_SAB_ ||DSAB_deadline_>0)&& datalen >= 1){
        goto send;
	}
...
//  {
//    sendpacket(seqno, rcv_nxt_, TH_ACK, 0, reason);
//	//sendpacket(seqno, rcv_nxt_, TH_FIN, 0, reason);
//    SAB_state_=-1;
//  }

	 //liu: added for DSAB
	 //if(DSAB_deadline_ > 0 && !syn && !fin && SAB_state_ != 2 && SAB_state_ != -1)
		// SAB_state_ = 3;
...
//}
  ...
  if(enable_SAB_ && DSAB_deadline_ <=0)
  {
    ...
    cwnd_ = wnd_;
    if (this->type_ == PT_ACK)
      this->rwnd_ = tcph->rwnd_;
  }

  //added for DSAB
  if(DSAB_deadline_>0)
  {
    //receiver side carry back window and winApply
    if (this->type_ == PT_ACK)
    {
      this->wndApply_=tcph->DSAB_wndApply_;
      this->wnd_=tcph->DSAB_wnd_;
      this->DSAB_type_=tcph->DSAB_type_;
    }else
    {
      this->DSAB_type_=tcph->DSAB_type_;
      switch (this->DSAB_type_)
      {
        //receive R_W feed back, should send confirmation
        case DSAB_R_W:
          this->wnd_ = tcph->DSAB_wnd_+wndBase_;
          cwnd_ = wnd_;
          this->wndApply_ = tcph->DSAB_wndApply_;
          this->SAB_state_=DSAB_STATE_SEND_BC;
          break;
        default:
          break;
      }
...
//		}
	...
	/*if (delack_interval_ > 0.0 &&
	 ...
	 // ws: added for D2TCP
	 if (d2tcp_ && d2tcp_deadline_>0) {
		 update_d2tcp_gamma(pkt);
		 if (d2tcp_gamma_ == 0)
			 cwnd_ += 1;
     }
	// Mohammad
...
FullTcpAgent::process_sack(hdr_tcp*)
{
	...
	return;
     ...
     //ws: added for D2TCP
     void
     FullTcpAgent::update_d2tcp_gamma(Packet *pkt){
         update_ecnhat_alpha(pkt);
         double d;
//   double B = d2tcp_total_bytes_ - ndatabytes_;
         double B = (infinite_send_) ? TCP_MAXSEQ :
                    curseq_ - highest_ack_ + 1;
         if (B <= 0)
         {
             d = 0.5;
...
//   double D = (d2tcp_deadline_ - Scheduler::instance().clock())*1000;
         double D = start_time + d2tcp_deadline_ - now();
         if (D <= 0)
         {
             d = 2.0;
         }else{
         ...
         d2tcp_gamma_ = pow(ecnhat_alpha_, d);
     }
